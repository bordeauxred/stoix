...
def learner_setup(
    ...
    # Skip non-array types (e.g., strings in optimizer state from AdamO)
    broadcast = lambda x: jnp.broadcast_to(x, (config.arch.update_batch_size,) + x.shape) if hasattr(x, 'shape') else x
