"""TD3 with vmapped multi-seed training and per-seed logging.

This variant runs multiple seeds in parallel on a single GPU using vmap,
while preserving per-seed metrics for logging. Each seed trains independently
(no gradient sharing across seeds).

Key differences from ff_td3.py:
- Each seed has its own independent parameters (no broadcast + pmean)
- Metrics are returned with seed dimension preserved
- Per-seed logging after JAX compilation

Usage:
    python stoix/systems/ddpg/ff_td3_multiseed.py \
        env=brax/halfcheetah \
        arch.update_batch_size=3 \
        arch.seeds=[42,43,44]
"""
...
def get_warmup_fn(
    ...
    """Get warmup function - separate params per seed."""
    ...
    # Vmap over seeds - each seed has its own params
    batched_warmup_step: Callable = jax.vmap(
        warmup, in_axes=(0, 0, 0, 0, 0), out_axes=(0, 0, 0, 0), axis_name="seed"
    )
...
def get_learner_fn(
    ...
    """Get the learner function with independent per-seed training."""
    ...
    def _update_step(
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            # NOTE: No pmean across "seed" axis - each seed trains independently!
            # Only pmean across device axis if using multi-device
    ...
    def learner_fn(
        ...
        """Learner function with independent per-seed training.

        Vmaps over seeds, but each seed trains independently (no gradient sharing).
        Metrics are returned with the seed dimension preserved.
        """

        # Vmap over seeds - axis_name="seed" instead of "batch"
        batched_update_step = jax.vmap(_update_step, in_axes=(0, None), axis_name="seed")
...
def learner_setup(
    ...
    Each seed gets its own independently initialized parameters.
    """
    # Get available TPU cores.
    n_devices = len(jax.devices())
    num_seeds = config.arch.update_batch_size  # Repurpose update_batch_size as num_seeds
    ...
    # PRNG keys - one per seed
    key = keys[0]
    seed_keys = jax.random.split(key, num_seeds * 2)
    actor_net_keys = seed_keys[:num_seeds]
    q_net_keys = seed_keys[num_seeds:]
    ...
    # Initialize params for each seed independently (vmapped init)
    def init_single_seed(actor_key, q_key):
        actor_online_params = actor_network.init(actor_key, init_x)
        ...
        q_online_params = double_q_network.init(q_key, init_x, init_a)
    ...
    # Vmap over seed keys to get per-seed params
    all_params, all_opt_states = jax.vmap(init_single_seed)(actor_net_keys, q_net_keys)
    ...
    # Each seed gets its own buffer
    config.system.buffer_size = config.system.total_buffer_size // (n_devices * num_seeds)
    config.system.batch_size = config.system.total_batch_size // (n_devices * num_seeds)
    ...
    # Initialize buffer state for each seed
    buffer_states = jax.vmap(lambda _: buffer_fn.init(dummy_transition))(jnp.arange(num_seeds))

    # Get batched iterated update and replicate it to pmap it over cores.
    learn = get_learner_fn(env, apply_fns, update_fns, buffer_fns, config)
    learn = jax.pmap(learn, axis_name="device")

    warmup = get_warmup_fn(env, actor_network_apply_fn, buffer_fn.add, config)
    warmup = jax.pmap(warmup, axis_name="device")

    # Initialise environment states and timesteps: across devices and seeds.
    key, *env_keys = jax.random.split(
        key, n_devices * num_seeds * config.arch.num_envs + 1
    )
    env_states, timesteps = env.reset(jnp.stack(env_keys))
    reshape_states = lambda x: x.reshape(
        (n_devices, num_seeds, config.arch.num_envs) + x.shape[1:]
    )
    # (devices, num_seeds, num_envs, ...)
    ...
    if config.logger.checkpointing.load_model:
        ...
        restored_params, _ = loaded_checkpoint.restore_params(input_params=all_params)
        all_params = restored_params

    # Define params to be replicated across devices (but NOT across seeds - each seed has its own).
    key, step_key, warmup_key = jax.random.split(key, num=3)
    step_keys = jax.random.split(step_key, n_devices * num_seeds)
    warmup_keys = jax.random.split(warmup_key, n_devices * num_seeds)
    reshape_keys = lambda x: x.reshape((n_devices, num_seeds) + x.shape[1:])
    step_keys = reshape_keys(jnp.stack(step_keys))
    warmup_keys = reshape_keys(jnp.stack(warmup_keys))

    # Replicate across devices only (not seeds - each seed has its own params)
    replicate_learner = (all_params, all_opt_states, buffer_states)
    replicate_learner = flax.jax_utils.replicate(replicate_learner, devices=jax.devices())

    # Initialise learner state.
    params, opt_states, buffer_states = replicate_learner

    # Warmup the buffer for each seed.
    env_states, timesteps, keys, buffer_states = warmup(
        params, env_states, timesteps, buffer_states, warmup_keys
    )
...
def run_experiment(_config: DictConfig) -> float:
    """Runs experiment with per-seed logging."""
    config = copy.deepcopy(_config)

    # Calculate total timesteps.
    n_devices = len(jax.devices())
    config.num_devices = n_devices
    num_seeds = config.arch.update_batch_size

    # Get seed values for logging (arch.seeds is added via +arch.seeds=[...] override)
    seed_values = list(OmegaConf.select(config, "arch.seeds", default=list(range(num_seeds))))
    if len(seed_values) != num_seeds:
        seed_values = list(range(num_seeds))
    ...
    steps_per_seed_per_rollout = steps_per_rollout // num_seeds

    # Logger setup
    logger = StoixLogger(config)
    logger.log_config(OmegaConf.to_container(config, resolve=True))
    print(f"{Fore.YELLOW}{Style.BRIGHT}JAX Global Devices {jax.devices()}{Style.RESET_ALL}")
    print(f"{Fore.CYAN}{Style.BRIGHT}Running {num_seeds} seeds in parallel: {seed_values}{Style.RESET_ALL}")
