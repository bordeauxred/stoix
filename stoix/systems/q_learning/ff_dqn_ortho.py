"""DQN with Gram orthogonalization regularization for isometric networks.

This variant adds Gram regularization to maintain approximate orthonormality
of weight matrices, enabling stable training of very deep networks (100+ layers).

The Gram regularization loss encourages:
- Wide matrices (n_in < n_out): WW^T ≈ (n_out/n_in) * I
- Tall/Square matrices (n_in >= n_out): W^TW ≈ I

Combined with 1-Lipschitz activations like GroupSort, this creates
isometric networks with bounded Lipschitz constants.

Usage:
    python stoix/systems/q_learning/ff_dqn_ortho.py \
        env=gymnax/cartpole \
    ...
    def _update_step(
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _q_loss_fn(
                ...
                # Compute Gram regularization loss
            ...
            logger.log(
                {
                    ...
                    f"seed_{seed}/ortho_loss": float(jnp.mean(all_metrics["ortho_loss"][i])),
                    f"seed_{seed}/gram_deviation": float(jnp.mean(all_metrics["gram_deviation"][i])),
                },
...
@hydra.main(
    ...
    config_name="default_ff_dqn_ortho.yaml",
    version_base="1.2",
)
def hydra_entry_point(cfg: DictConfig) -> float:
    ...
    if multiseed is not None:
        ...
        layer_sizes = OmegaConf.select(
            cfg, "network.actor_network.pre_torso.layer_sizes", default=[256, 256]
        )
        depth = len(layer_sizes) if layer_sizes else 2
        activation = OmegaConf.select(
            cfg, "network.actor_network.pre_torso.activation", default="silu"
        )
        ...
        group_name = f"dqn_ortho_{env_name}_d{depth}_{activation}_l{ortho_lambda}_{timestamp}"
        ...
        print(f"{Fore.CYAN}{Style.BRIGHT}DQN Ortho multi-seed experiment completed{Style.RESET_ALL}")
        ...
        print(f"{Fore.CYAN}{Style.BRIGHT}DQN Ortho experiment completed{Style.RESET_ALL}")
